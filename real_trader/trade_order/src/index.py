import re
import json
from real_trader.trade_order.src.zt_clienttrader import ZTClientTrader
from real_trader.trade_order.src.mail import Mail
...
class ZhongTaiTrader:
    ...
    def run_email(self, account, password, form_account):
        ...
        if account and password and form_account :
    ...
    def login(self, account, password, sales_office_id):
        self.trader.login(account, password, sales_office_id)
    ...
    def position(self):
        ...
        if data.get('总资产') == '-':
            portfolio = {
                'total_value': data.get('总资产'),
                'available_cash': data.get('可用金额'),
                'transferable_cash': data.get('可取金额'),
                'positions_value': data.get('股票市值'),
