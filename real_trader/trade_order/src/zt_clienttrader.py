...
from real_trader.trade_order.src.grid_strategy import IGridStrategy
from real_trader.trade_order.src.grid_strategy import Copy

from real_trader.trade_order.src.pop_dialog_handler import TradePopDialogHandler
from real_trader.trade_order.src.pop_dialog_handler import PopDialogHandler

import time
import easyutils

from real_trader.trade_order.src.client import *
from real_trader.trade_order.src.ocr import *
...
class ZTClientTrader():
    ...
    def login(self, account, password, sales_office_id):

        # 选择营业部
        self._main.ComboBox2.select(sales_office_id)

        ## 输入账号
        edit = self._main.children(class_name='Edit')[0]
        while edit.element_info.rich_text != account:
            edit.set_edit_text('', 0, 100)
            edit.type_keys(account)
            time.sleep(0.1)
        time.sleep(0.1)
        ...
        while wins.element_info.rich_text != code:
            wins.set_edit_text('', 0, 100)
            wins.type_keys(code)
            time.sleep(0.1)
