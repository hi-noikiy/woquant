...
class _CurrentDic(dict):
    ...
    def __missing__(self, code):
        info = _global['session'].get(
            'https://stock.xueqiu.com/v5/stock/quote.json?extend=detail&symbol=' + parse_code(code),
            cookies=_global['cookies'], headers=headers).json()
...
def init_schedudler():
    ...
    schedudler.add_job(func=_global['before_trading_start'], args=[_global['context']], trigger='cron', hour=9,
                       minute=9, day_of_week='mon-fri')
    schedudler.add_job(func=_global['after_trading_end'], args=[_global['context']], trigger='cron', hour=15, minute=30,
                       day_of_week='mon-fri')
...
def get_ticks(security, end_dt, count, start_dt=None):
    ...
    if count > 100:
        ...
        info = _global['session'].get(
            'https://stock.xueqiu.com/v5/stock/history/trade.json?symbol=' + parse_code(security) + '&count=' + str(
                count), cookies=_global['cookies'], headers=headers).json()
...
def get_current_tick(stock, df=False):
    info = _global['session'].get('https://stock.xueqiu.com/v5/stock/realtime/quotec.json?symbol=' + parse_code(stock),
                                  cookies=_global['cookies'], headers=headers).json()
    quote = info['data'][0]
    stock = quote['symbol']
    result = {
...
def _get_current_tick():
    ...
    if len(stock_list):
        ...
        info = _global['session'].get(
            'https://stock.xueqiu.com/v5/stock/realtime/quotec.json?symbol=' + ','.join(stocks),
            cookies=_global['cookies'], headers=headers).json()
        quotes = info['data']
...
def run_daily(event, time):
    ...
    schedudler.add_job(func=event, args=[_global['context']], trigger='cron', hour=hour, minute=minute,
                       day_of_week='mon-fri')
    schedudler.start()
