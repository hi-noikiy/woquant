...
from real_trader.trade_order.src.index import ZhongTaiTrader, X
...
# 实时数据模块 [tick相关函数覆盖]
from real_trader.trade_bundle.live_trade import get_current_tick
...
# 初始化实盘模块
def init_trader(g, context, account, password, sales_office_id, path):
    logger.info('##### 初始化实盘模块 #####')
    _order_global['trader'] = ZhongTaiTrader()
    _order_global['g'] = g
    _order_global['context'] = context
    logger.info('准备登录资金账号 {}'.format(account))
    run_client(path)
    login(account, password, sales_office_id)
    time.sleep(1)
...
# 登陆账号
def login(userId, password, sales_office_id=1):
    info = _order_global['trader'].login(userId, password, sales_office_id)
    ...
    if amount == 0:
        ...
        if amount != 0:
            ...
            if 'message' in result:
                print('委托函数 委托失败 标的: {} 委托方向: {} 委托类型: {} 数量: {} 回报信息: {}'.format(security,
                                                                                  'Buy' if amount > 0 else 'Sell',
                                                                                  style, amount, result['message']))
            else:
                print('委托函数 委托成功 标的: {} 委托方向: {} 委托类型: {} 数量: {} 委托编号: {}'.format(security,
                                                                                  'Buy' if amount > 0 else 'Sell',
                                                                                  style, amount, result['entrust_id']))
